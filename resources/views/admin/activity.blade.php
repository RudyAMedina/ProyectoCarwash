                        ...
                        <!-- Mensajes de estado -->
                            ...
                            <div class="w-full max-w-7xl mx-auto px-4 overflow-x-auto">
                                <table class="w-full table-auto bg-white border border-gray-300">
                                        ...
                                        <tbody>
                                            @forelse ($activity as $activitys)
                                                <tr>
                                                    <td class="py-2 px-4 border">{{ $activitys->id }}</td>
                                                    <td class="py-2 px-4 border">{{ $activitys->title }}</td>
                                                    <td class="py-2 px-4 border">{{ $activitys->description }}</td>
                                                    <td class="py-2 px-4 border"><img src="{{ asset('images/' . $activitys->image_path) }}" alt="Imagen" class="w-48 h-auto"></td>
                                                    <td class="py-2 px-4 border">{{ $activitys->activity_date }}</td>
                                                    <td class="py-2 px-4 border text-center">
                                                        <a href="{{ route('activity.edit', $activitys->id) }}" class="text-blue-500 hover:underline mr-2">Editar</a>
                                                        <form action="{{ route('activity.destroy', $activitys->id) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de eliminar este servicio?');">
                                                            @csrf
                                                            @method('DELETE')
                            ...
                            <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" x-cloak>
                                <div class="bg-white rounded-lg shadow-lg w-full sm:w-[500px] p-6 max-h-[90%] overflow-y-auto">
                                    <div class="flex justify-between items-center mb-4">
                                        <h2 class="text-xl font-semibold">Agregar Publicación</h2>
                                    ...
                                    <form method="POST" action="{{ route('activity.store') }}" enctype="multipart/form-data">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input type="file" name="image_path" id="imageInput" accept="image_path/*" class="w-full border rounded px-3 py-2">
                                            ...
                                            <div id="previewContainer" class="mt-4 hidden">
                                                <p class="text-sm text-gray-600 mb-1">Vista previa de la imagen:</p>
                                                <img id="imagePreview" src="" class="w-48 h-48 object-cover rounded shadow">
                                            </div>
                                        </div>
                                        <div class="mb-4">
                                            <label class="block text-sm font-medium">Fecha</label>
                                            <input type="date" name="activity_date" class="w-full border rounded px-3 py-2" value="{{ old('activity_date') }}" required>
                                        </div>
...
<script>
    //Función para tener una vista previa de la imagen
